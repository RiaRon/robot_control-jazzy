...
setsid ros2 launch openarm_bringup openarm.launch.py \
...
if ! kill -0 "$LAUNCH_PID" 2>/dev/null; then
    echo "error: OpenArm fake-hardware launch exited unexpectedly" >&2
    exit 1
fi
...
echo "OpenArm fake-hardware smoke test passed"
