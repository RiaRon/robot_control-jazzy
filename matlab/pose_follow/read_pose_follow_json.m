...
outerRawCandidateRad = nan(sampleCount, jointCount);
outerBoundedCandidateRad = nan(sampleCount, jointCount);
outerClampMask = false(sampleCount, jointCount);
outerCrossingMask = false(sampleCount, jointCount);
outerTargetHoldMask = false(sampleCount, jointCount);
outerOutwardBlockedMask = false(sampleCount, jointCount);
outerStalledRecoveryMask = false(sampleCount, jointCount);
outerTargetReversalMask = false(sampleCount, jointCount);
outerReversalBlockedMask = false(sampleCount, jointCount);
postCrossingCommandMask = false(sampleCount, jointCount);
postCrossingMeasuredMask = false(sampleCount, jointCount);
postCrossingIkTargetRad = nan(sampleCount, jointCount);
postCrossingActiveCommandRad = nan(sampleCount, jointCount);
postCrossingRawCandidateRad = nan(sampleCount, jointCount);
postCrossingPreLimiterTargetRad = nan(sampleCount, jointCount);
postCrossingCartesianCandidateRad = nan(sampleCount, jointCount);
postCrossingCommandRad = nan(sampleCount, jointCount);
postCrossingUsedIkTargetMask = false(sampleCount, jointCount);
postCrossingHoldMask = false(sampleCount, jointCount);
postCrossingReleaseMask = false(sampleCount, jointCount);
postCrossingMeasuredRad = nan(sampleCount, jointCount);
...
for sampleIndex = 1:sampleCount
    ...
    outerRawCandidateRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'raw_candidate_rad');
    outerBoundedCandidateRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'bounded_candidate_rad');
    outerClampMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', 'clamp_mask');
    outerCrossingMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', 'crossing_mask');
    outerTargetHoldMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', 'target_hold_mask');
    outerOutwardBlockedMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'outward_accumulation_blocked_mask');
    outerStalledRecoveryMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'stalled_recovery_mask');
    outerTargetReversalMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'target_direction_reversal_mask');
    outerReversalBlockedMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_target_crossing_clamp', ...
        'target_reversal_outward_blocked_mask');
    postCrossingCommandMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'command_crossing_mask');
    postCrossingMeasuredMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'measured_crossing_mask');
    postCrossingIkTargetRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', 'ik_target_rad');
    postCrossingActiveCommandRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'active_command_before_rad');
    postCrossingRawCandidateRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'raw_candidate_rad');
    postCrossingPreLimiterTargetRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'pre_limiter_target_rad');
    postCrossingCartesianCandidateRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'cartesian_limited_candidate_rad');
    postCrossingCommandRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'post_limiter_command_rad');
    postCrossingUsedIkTargetMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'used_ik_target_mask');
    postCrossingHoldMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'target_hold_mask');
    postCrossingReleaseMask(sampleIndex, :) = nestedLogicalVector( ...
        sample, jointCount, 'outer_post_crossing_hold', ...
        'target_changed_release_mask');
    postCrossingMeasuredRad(sampleIndex, :) = nestedVector( ...
        sample, jointCount, 'outer_post_crossing_hold', 'measured_rad');
...
run.outer_target_crossing_clamp = struct( ...
    'raw_candidate_rad', outerRawCandidateRad, ...
    'bounded_candidate_rad', outerBoundedCandidateRad, ...
    'clamp_mask', outerClampMask, ...
    'crossing_mask', outerCrossingMask, ...
    'target_hold_mask', outerTargetHoldMask, ...
    'outward_accumulation_blocked_mask', outerOutwardBlockedMask, ...
    'stalled_recovery_mask', outerStalledRecoveryMask, ...
    'target_direction_reversal_mask', outerTargetReversalMask, ...
    'target_reversal_outward_blocked_mask', outerReversalBlockedMask, ...
    'summary', fieldOr(raw.result, ...
        'outer_target_crossing_clamp', struct()));
run.outer_post_crossing_hold = struct( ...
    'command_crossing_mask', postCrossingCommandMask, ...
    'measured_crossing_mask', postCrossingMeasuredMask, ...
    'ik_target_rad', postCrossingIkTargetRad, ...
    'active_command_before_rad', postCrossingActiveCommandRad, ...
    'raw_candidate_rad', postCrossingRawCandidateRad, ...
    'pre_limiter_target_rad', postCrossingPreLimiterTargetRad, ...
    'cartesian_limited_candidate_rad', postCrossingCartesianCandidateRad, ...
    'post_limiter_command_rad', postCrossingCommandRad, ...
    'used_ik_target_mask', postCrossingUsedIkTargetMask, ...
    'target_hold_mask', postCrossingHoldMask, ...
    'target_changed_release_mask', postCrossingReleaseMask, ...
    'measured_rad', postCrossingMeasuredRad, ...
    'summary', fieldOr(raw.result, 'outer_post_crossing_hold', struct()));
...
function vector = nestedLogicalVector(parent, width, containerName, fieldName)
vector = false(1, width);
if ~isstruct(parent) || ~isfield(parent, containerName)
    return;
end
container = parent.(containerName);
