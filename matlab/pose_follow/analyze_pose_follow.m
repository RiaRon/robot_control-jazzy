...
experimentTemplate = struct( ...
    ...
    'termination', "", 'is_partial', false, 'refusal', struct(), ...
    'ready_posture', struct(), 'comparison_metadata', struct(), ...
...
row = struct( ...
    'experiment', "", 'source_file', "", 'schema_version', nan, ...
    'schema_variant', "", 'group', "", 'profile', "", ...
    'termination', "", 'is_partial', false, ...
    'ready_posture_name', "", 'ready_check_passed', false, ...
    'ready_start_max_error_rad', nan, ...
    'refusal_reason', "", 'refused_sequence', nan, ...
    'refusal_phase', "", ...
    ...
    'ik_continuity_exhausted', nan, ...
    'ik_candidate_count', nan, 'ik_rejected_candidate_count', nan, ...
    'ik_selected_cost_mean', nan, 'ik_selected_cost_max', nan, ...
...
row.termination = run.termination;
row.is_partial = run.is_partial;
row.ready_posture_name = run.ready_posture.name;
row.ready_check_passed = run.ready_posture.passed;
if any(isfinite(run.ready_posture.start_error_rad))
    row.ready_start_max_error_rad = max(abs(run.ready_posture.start_error_rad), [], 'omitnan');
end
if run.refusal.available
    row.refusal_reason = run.refusal.reason;
    row.refused_sequence = run.refusal.refused_sequence;
    row.refusal_phase = run.refusal.profile_phase;
end
row.phase = phaseName;
if phaseName == "all"
...
if phaseName == "all" || hasTiming
    if phaseName == "all"
        ...
        row.ik_continuity_rejected = run.ik.continuity_rejected;
        row.ik_continuity_retries = run.ik.continuity_retries;
        row.ik_continuity_exhausted = run.ik.continuity_exhausted;
        row.ik_candidate_count = run.ik.candidate_count;
        row.ik_rejected_candidate_count = run.ik.rejected_candidate_count;
        if ~isempty(run.ik.selection_events)
            selectedCosts = [run.ik.selection_events.selected_cost];
            selectedCosts = selectedCosts(isfinite(selectedCosts));
            if ~isempty(selectedCosts)
                row.ik_selected_cost_mean = mean(selectedCosts);
                row.ik_selected_cost_max = max(selectedCosts);
            end
        end
    else
...
summary.termination = run.termination;
summary.is_partial = run.is_partial;
summary.refusal = run.refusal;
summary.ready_posture = run.ready_posture;
summary.comparison_metadata = struct( ...
    'ready_posture_name', run.ready_posture.name, ...
    'ready_check_passed', run.ready_posture.passed, ...
    'profile', run.profile, 'group', run.group);
...
summary.ik = struct( ...
    ...
    'solve_attempts', run.ik.solve_attempts, ...
    'candidate_count', run.ik.candidate_count, ...
    'rejected_candidate_count', run.ik.rejected_candidate_count, ...
    'selection_events', run.ik.selection_events, ...
    'continuity_rejected', run.ik.continuity_rejected, ...
    'continuity_retries', run.ik.continuity_retries, ...
    'continuity_exhausted', run.ik.continuity_exhausted, ...
