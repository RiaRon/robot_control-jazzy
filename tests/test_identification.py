...
def _staircase_errors(kp, coulomb, gravity, values, steps):
    """What a spring with dry friction reads at each torque in *values*."""
    errors, held = [], None
...
def _staircase_sweep(kp, coulomb, gravity, peak=0.6, steps=5):
    ...
    errors = _staircase_errors(kp, coulomb, gravity, values, steps)
    return GravitySweep(
        group="openarm_right_arm",
        joint_names=("r_aj_5",),
...
def _multi_joint_staircase_sweep(kp, coulomb, gravity, peak=0.6, steps=5):
    """Every joint of a group in one file, which is what `pose torque` writes.

    `--joint` defaults to the whole group and `measure_staircase` drives them
    one after another, so a joint's own rounds are one contiguous block and the
    rest of the file is somebody else's staircase. Through those the joint
    carries no applied torque and never moves, so it reads one repeated
    standing error. A joint nothing has pushed sits anywhere inside its
    stiction band; the middle of it, its undeflected gravity droop, is the one
    place that belongs to neither branch's line.

    Each block is generated by the same function `_staircase_sweep` uses, so a
    fitter that looks only at the joint's own rounds must reach exactly the
    single-joint numbers.
    """
    from robot_control.excitation import staircase

    values = staircase(peak, steps)
    width = len(kp)
    standing = [g / k for g, k in zip(gravity, kp)]
    applied_rows, error_rows = [], []
    for driven in range(width):
        block = _staircase_errors(
            kp[driven], coulomb[driven], gravity[driven], values, steps
        )
        for index, value in enumerate(values):
            applied = np.zeros(width)
            applied[driven] = value
            errors = np.array(standing, dtype=float)
            errors[driven] = block[index]
            applied_rows.append(applied)
            error_rows.append(errors)
    rounds = len(applied_rows)
    return GravitySweep(
        group="openarm_right_arm",
        joint_names=tuple(f"r_aj_{index + 5}" for index in range(width)),
        poses=np.zeros((rounds, width)),
        modelled_torque=np.zeros((rounds, width)),
        scales=np.zeros((rounds, width)),
        applied_torque=np.array(applied_rows),
        errors=np.array(error_rows),
    )


def test_a_joint_is_fitted_from_its_own_rounds_and_not_the_whole_file():
    """Splitting on one argmax over the file puts every other joint's rounds —
    zero applied torque, one repeated standing error — into this joint's
    branches. They land on both sides at once, which drags the two intercepts
    together, and the gap between those intercepts is Fc.

    Run against the single argmax: joint 0, whose block comes first, takes all
    nine foreign rounds into its falling branch and reads Fc 0.0662 against a
    true 0.10 (-34%), kp 15.63 against 15.0 (+4%). Joint 1 takes them into its
    rising branch and reads Fc 0.0543 (-32%); its kp survives only because
    that branch is symmetric about zero torque, so nine more points at zero
    leave the slope alone and move the intercept only — which is to say the
    quantity that collapses is exactly the one hdgp_export writes out as
    joint_friction.
    """
    kp, coulomb = [15.0, 11.0], [0.10, 0.08]
    sweep = _multi_joint_staircase_sweep(
        kp=kp, coulomb=coulomb, gravity=[0.20, 0.10]
    )

    estimate = fit_staircase([sweep])

    assert not estimate.unidentifiable
    np.testing.assert_allclose(estimate.stiffness, kp, rtol=1e-6)
    np.testing.assert_allclose(estimate.coulomb_nm, coulomb, rtol=1e-6)


def test_a_single_joint_file_fits_the_same_numbers_it_always_did():
    """The window that keeps a joint to its own rounds must be a no-op on a
    file holding one joint: its first and last rounds are both at full torque,
    so the window is the whole file. Pinned to the numbers this fixture gave
    before the window existed, tightly enough that a behaviour change shows."""
    estimate = fit_staircase([_staircase_sweep(kp=15.0, coulomb=0.08, gravity=0.2)])

    assert estimate.stiffness[0] == pytest.approx(15.0, rel=1e-12)
    assert estimate.coulomb_nm[0] == pytest.approx(0.08, rel=1e-12)
    assert estimate.bias_nm[0] == pytest.approx(0.2, rel=1e-12)
    assert estimate.used[0] == 9
